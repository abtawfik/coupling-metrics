...
!-----------------------------------------------------------------------------
      ...
      do nn=1,nsegments
         ...
         moist_lapse  =  (grav/cp) * ( (1. + (Lv    * wmr(pmid/1e2,tmid-C2K)*1e-3)/(   Rd*tmid   )) /   &
                                       (1. + (Lv**2 * wmr(pmid/1e2,tmid-C2K)*1e-3)/(cp*Rv*tmid**2)) )
...
!
!   Author:
!   Original F77 code -  Schlatter Baker  May 17 1982
!   Converted to F90  -  Ahmed Tawfik     Jul  1 2015
!----------------------------------------------------------------------------------
  real(4) function wmr(p,t)

   implicit none
!
! Input variables
!
   real(4), intent(in) :: t, p
!
! Local variables
!
   real(4), parameter  :: eps = 0.62197, es0 = 6.1078
   real(4)             :: x, wfw, fwesw, r, pol, esw
!----------------------------------------------------------------------------------
        ...
        !--- Calcualte saturation vapor pressure returned in (mb)
        !--- using temperature inputs in degrees C.
        !--- This follows the Wobus polynomial approximation and
        !--- is valid for temperature ranges -50 to 100 Celcius
