...
!-----------------------------------------------------------------------------
      ...
      !if( all(ne.eq.missing) ) then
      !  do tt=2001,4020
      !     write(*,FMT1) dRH_estimate(tt), pbl_growth(tt), pbl_heating(tt), dry_entrain(tt), C_theta(tt), pblh_mid(tt), shf(tt), lhf(tt)
      !     write(*,*) dRH_estimate(tt)*1e2, (relh(tt+1)-relh(tt))*1e2, (relh(tt)-relh(tt-1))*1e2
      !     do zz=1,nlev
      !        write(*,*) zz, press_in(tt,zz), tmp_in(tt,zz), qhum_in(tt,zz) 
      !     end do
      !     write(*,*) rho2m  (tt), t2m(tt), q2m (tt), C_theta(tt)
      !     write(*,*) shf0   (tt), shf(tt), lhf0(tt), lhf    (tt)
      !     write(*,*) shf_ent(tt), bowen_s(tt)
      !     write(*,*)
      !     write(*,*)
      !     write(*,*)
      !  end do
