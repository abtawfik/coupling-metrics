...
!
!---------------------------------------------------------------------------------
  subroutine terra_wrapper( nlon,  nlat,  ntim,  soilm,  flux,  tcp,  missing )
   implicit none

!
! Input/Output Variables
!
   integer, intent(in   )                              ::  nlat, nlon     ! *** spatial dimensions
   integer, intent(in   )                              ::  ntim           ! *** size of time dimension

   real(4), intent(in   )                              ::  missing        ! *** missing value - useful for obs
   real(4), intent(in   ), dimension(nlon,nlat,ntim)   ::  soilm          ! *** soil moisture - (time,space)
   real(4), intent(in   ), dimension(nlon,nlat,ntim)   ::  flux           ! *** surface flux  - (time,space)
   real(4), intent(out  ), dimension(nlon,nlat     )   ::  tcp            ! *** terrestrial coupling parameter [flux units]
!
! Local variables
!
   integer                      ::  xx, yy
...
!-----------------------------------------------------------------------------
      !-----------------------------------------------------------------------------
      !-- Loop over all non-level dimensions after initialization
      !-----------------------------------------------------------------------------
      tcp   = missing
      do yy = 1,nlat
      do xx = 1,nlon
         call terra_coupling( ntim, soilm(xx,yy,:), flux(xx,yy,:), tcp(xx,yy), missing )
      end do
      end do

  end subroutine terra_wrapper
...
!---------------------------------------------------------------------------------
  subroutine terra_coupling( ntim,  soilm,  flux,  tcp,  missing )
   implicit none
...
!
   ...
   real(4), intent(in   )                    ::  missing        ! *** missing value - useful for obs
   real(4), intent(in   ), dimension(ntim)   ::  soilm          ! *** soil moisture - (time,space)
   real(4), intent(in   ), dimension(ntim)   ::  flux           ! *** surface flux  - (time,space)
   real(4), intent(out  )                    ::  tcp            ! *** terrestrial coupling parameter [flux units]
!
! Local variables
!
   ...
   integer                      ::  n1
   integer                      ::  nsample
   real(4), dimension(ntim)     ::  soilm_mean                       
   real(4), dimension(ntim)     ::  soilm_diff     
   real(4)                      ::  soilm_std      
   real(4)                      ::  flux_std
   real(4)                      ::  flux_std_with_n
   real(4), dimension(ntim)     ::  flux_mean                       
   real(4), dimension(ntim)     ::  flux_diff     
   real(4)                      ::  covar          
